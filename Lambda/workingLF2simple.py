...
def search_photos_by_labels(endpoint, index, labels, size):
    logger.debug(endpoint)
    logger.debug(index)

    response = requests.post(endpoint + "/" + index + "/Photo/_search", json=compose_query(labels, size))
    return decode_response(response.text)

def compose_query(labels, size):
    query = {
        "size": str(size),
        "from": "0",
        "version": True,
